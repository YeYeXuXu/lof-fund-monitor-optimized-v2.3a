...
- `.github/workflows/fund-monitor.yml`：每周一到周五北京时间 09:30 启动工作流，默认 15:00 停止。
...
> 注意：GitHub Actions 的 `schedule.cron` 默认按 UTC 解释；本项目使用 `timezone: Asia/Shanghai`，直接按北京时间工作日 09:30 触发，避免手动 UTC 换算错误。
...
```yaml
schedule:
  # Beijing time, Monday-Friday 09:30.
  - cron: '30 9 * * 1-5'
    timezone: Asia/Shanghai
```

含义是：每周一到周五北京时间 09:30 触发工作流；手动运行时也可以通过 `run_from` 指定开始时间。
