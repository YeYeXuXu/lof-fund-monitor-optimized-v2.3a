...
- v2.3a 新增：启动时自动使用 AkShare `fund_etf_spot_em` 快照筛选并新增“可直接取得 `IOPV实时估值` 与 `基金折价率`”的 ETF/场内交易基金；仅补充数据库缺失代码，不覆盖用户已配置基金。可通过环境变量 `AUTO_ADD_AKSHARE_DIRECT_FUNDS=0` 关闭自动补充。
- v2.3a 新增：后台“添加基金”优先使用 AkShare 快照识别基金名称和交易所；对于可直接取得 `f441=IOPV实时估值` 与 `f402=基金折价率` 的基金，自动采用 AkShare 交易所映射，添加后沿用现有 AkShare 优先、项目原方法兜底的数据刷新链路。
- v2.3a 修复：`fund_value_estimation_em`（LOF/场内交易基金/QDII）超时改为“可选估值源跳过”的 INFO 日志，不再作为告警级报错，也不会阻塞行情刷新和微信定时推送。
- v2.3a 增强：AkShare `fund_purchase_em` 除申购/赎回状态外，同步解析“最新净值/报告时间”，在估值接口不可用时仍可优先从 AkShare 获取官方单位净值。
- v2.3a 修复：AkShare `f402=基金折价率` 为“折价率”字段，已统一取反为项目折溢价率（折价为负、溢价为正）；LOF 因 AkShare `fund_lof_spot_em` 源码未提供 `f402/f441`，改为用“最佳估算净值/官方净值 + 交易价”计算，并在日志打印 `EstNAV` 与 `PremiumBase` 便于核对。
- v2.3a 增强：数据刷新增加 started / AkShare snapshot / progress / completed / waiting 日志；微信告警到点时若 AkShare 快照刷新锁被后台任务占用，会直接使用缓存快照和已保存实时数据，确保微信推送不等待刷新锁。
- v2.3a 修复：微信阈值告警正文中的申购/赎回状态优先使用 AkShare `fund_purchase_em` 批量接口；接口缺失、超时或单只基金未命中时，自动回退原东方财富 F10/基金页状态解析。
- v2.3a 调整：微信定时推送到点后不再等待后台数据刷新任务释放锁；推送使用已保存实时数据叠加快速 AkShare 快照，确保数据刷新不影响微信定时推送。
- v2.3a 明确：开盘/交易时段每 5 分钟刷新基金数据，休市/非交易时段每 30 分钟刷新；该刷新节奏与微信推送时间相互独立。
- v2.3a 修复：AkShare/EastMoney 行情接口增加 `push2`、`push2delay`、`88.push2`、`2.push2` 多主机兜底，并对断连、超时、空响应进行重试，降低 `ServerDisconnectedError` 导致整批 LOF 快照不可用的概率。
- v2.3a 增强：GitHub Actions/服务日志会输出每只基金的 `Source=[...]`，明确 NAV、估值、价格和折溢价来自 AkShare 还是原有接口/计算兜底。
...
- 微信阈值告警正文已增加版本号：`净值估值模型优化v2.3a`。
- v2.3a 增强：微信阈值告警正文中每只基金的折溢价率、交易价格、估算净值、成交金额后追加实际来源标签：`akshare`、`项目原方法` 或 `akshare+项目原方法`。
...
- v2.3a 微信推送只用于折溢价阈值告警，不再发送定时汇总或测试消息；支持多个 Server酱 SendKey 同时推送。
