...
- v2.3L 新增：复刻 AkShare `fund_etf_fund_daily_em` 场内交易基金实时数据方法，读取 AkShare/天天基金直接给出的 `折价率` 字段；启动/刷新时与 `fund_value_estimation_em` 的 `估算值` 取交集，自动新增“可直接取得折溢价率 + 估算净值”的非 ETF 场内基金，ETF 基金不纳入本次自动新增。
- v2.3L 新增：自动新增基金会按名称和类型尽量匹配 LOF 监控项目原有的 `market`、`algo_type`、`category`、代理指数配置；可用环境变量 `AKSHARE_AUTO_ADD_FUNDS=0` 关闭，或用 `AKSHARE_AUTO_ADD_FUNDS_MAX` 限制单次新增数量。
- v2.3L 修复：`fund_value_estimation_em`（LOF/场内交易基金/QDII）超时改为“可选估值源跳过”的 INFO 日志，不再作为告警级报错，也不会阻塞行情刷新和微信定时推送。
- v2.3L 增强：AkShare `fund_purchase_em` 除申购/赎回状态外，同步解析“最新净值/报告时间”，在估值接口不可用时仍可优先从 AkShare 获取官方单位净值。
- v2.3L 修复：AkShare `f402=基金折价率` 为“折价率”字段，已统一取反为项目折溢价率（折价为负、溢价为正）；LOF 因 AkShare `fund_lof_spot_em` 源码未提供 `f402/f441`，改为用“最佳估算净值/官方净值 + 交易价”计算，并在日志打印 `EstNAV` 与 `PremiumBase` 便于核对。
- v2.3L 增强：数据刷新增加 started / AkShare snapshot / progress / completed / waiting 日志；微信告警到点时若 AkShare 快照刷新锁被后台任务占用，会直接使用缓存快照和已保存实时数据，确保微信推送不等待刷新锁。
- v2.3L 修复：微信阈值告警正文中的申购/赎回状态优先使用 AkShare `fund_purchase_em` 批量接口；接口缺失、超时或单只基金未命中时，自动回退原东方财富 F10/基金页状态解析。
- v2.3L 调整：微信定时推送到点后不再等待后台数据刷新任务释放锁；推送使用已保存实时数据叠加快速 AkShare 快照，确保数据刷新不影响微信定时推送。
- v2.3L 明确：开盘/交易时段每 5 分钟刷新基金数据，休市/非交易时段每 30 分钟刷新；该刷新节奏与微信推送时间相互独立。
- v2.3L 修复：AkShare/EastMoney 行情接口增加 `push2`、`push2delay`、`88.push2`、`2.push2` 多主机兜底，并对断连、超时、空响应进行重试，降低 `ServerDisconnectedError` 导致整批 LOF 快照不可用的概率。
- v2.3L 增强：GitHub Actions/服务日志会输出每只基金的 `Source=[...]`，明确 NAV、估值、价格和折溢价来自 AkShare 还是原有接口/计算兜底。
...
- 微信阈值告警正文已增加版本号：`净值估值模型优化v2.3L`。
- v2.3L 增强：微信阈值告警正文中每只基金的折溢价率、交易价格、估算净值、成交金额后追加实际来源标签：`akshare`、`项目原方法` 或 `akshare+项目原方法`。
...
- v2.3L 微信推送只用于折溢价阈值告警，不再发送定时汇总或测试消息；支持多个 Server酱 SendKey 同时推送。
...
| 申购赎回状态 | 东方财富 / AkShare 同源接口 | `fund.eastmoney.com` / `fund_purchase_em` |
| 场内基金折价率与自动新增候选 | AkShare 同源天天基金场内交易基金表 | `fund_etf_fund_daily_em`（排除 ETF） |
| 净值估算 | AkShare 同源东方财富净值估算 | `fund_value_estimation_em` |
...
| `AKSHARE_AUTO_ADD_FUNDS` | `1` | 默认自动新增同时具备 AkShare 直接折价率和估算净值的非 ETF 场内基金；设为 `0` 关闭 |
| `AKSHARE_AUTO_ADD_FUNDS_MAX` | `0` | 单次自动新增基金数量上限；`0` 表示不限制 |
